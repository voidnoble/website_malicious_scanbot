...
const webHookUrl = "https://hooks.slack.com/services/xxxxxxxx/xxxxxxxxxx/xxxxxxxxxxxxxxxxxx";
...
cron.schedule('*/20 * * * *', () => {
    (async() => {
        ...
        if (footerNextScript) {
            ...
            if (isSafe == "NO") {
                msg = "웹사이트 article에 Case 001 악성코드 삽입 감지됨!";
        ...
        if (isSafe == "NO") {
            msg = "웹사이트 article에 Case 002 악성코드 삽입 감지됨!";
            webhook.send(msg, (err, res) => {
                if (err) {
                    console.log('Error:', err);
                    return;
        ...
        if (isSafe == "NO") {
            msg = "웹사이트 article에 Case 003 악성코드 삽입 감지됨!"+ msg;
            webhook.send(msg, (err, res) => {
                if (err) {
                    console.log('Error:', err);
                    return;
        ...
        if (isSafe == "NO") {
            msg = "웹사이트 article에 Case 004 악성코드 삽입 감지됨!" + msg;
            webhook.send(msg, (err, res) => {
                if (err) {
                    console.log('Error:', err);
                    return;
        ...
        console.log(`[${now}] article Is safe? ${isSafe}`);
        await browser.close();
    })();
});
