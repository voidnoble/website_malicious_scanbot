...
const webHookUrl = "https://hooks.slack.com/services/xxxxxxxxx/xxxxxxxxx/xxxxxxxxxxxxxxx";
...
cron.schedule('*/10 * * * *', () => {
    (async() => {
        ...
        try {
            await page.goto("http://www.domain.com");
        } catch (exception) {
            console.log(exception);
            await browser.close();
            return;
        }
        let body = await page.evaluate(() => document.body.outerHTML);
        const $ = cheerio.load(body);
        let msg = "";
        let isSafe = "YES";
        let functionPattern = /function (\w+) ?\(/g;
        let functionWhitelists = ["goto_tab","showFlash","getFlashHtml","hideTopBanner","jq__slideUpAndRemove","isMsIe","isMsIe11","open_win","close_win","scroll_realtimenews","openPopup","pre_img","next_img","set_slide","img_resize","comment_length_check","submit_comment_form","print_article","goto_page","twitter_login","facebook_login","me2day_login","sns_login_success","Prism_PlayerSetting","responseAdparamData","setPlayData","Prism_onStateEventHandler","Prism_finishEventHandler","newsTab"];
        
        let scriptSrcWhitelistPattern = /www\.google-analytics\.com|www\.googletagmanager\.com|www\.googletagservices\.com|ajax\.googleapis\.com|adservice\.google\.com|adservice\.google\.co\.kr|securepubads\.g\.doubleclick\.net|cdnjs\.cloudflare\.com|compass\.adop\.cc|de\.tynt\.com|cdn\.tynt\.com|whos\.amung\.us|t\.dtscout\.com|js\.keywordsconnect\.com|js2\.keywordsconnect\.com|cm\.keywordsconnect\.com|waust\.at|mktag\.mt\.co\.kr|ssp\.realclick\.co\.kr|interface\.interworksmedia\.co\.kr|cm\.interworksmedia\.co\.kr|ds\.interworksmedia\.co\.kr|ps\.eyeota\.net|n-cdn\.areyouahuman\.com|t\.dtscdn\.com|ad\.impactify\.io|api\.dmcdn\.net|ad\.3dpop\.kr/;
        ...
        if (footerNextScript) {
            ...
            if (isSafe == "NO") {
                msg = "웹사이트에 Case 001 악성코드 삽입 감지됨!";
        ...
        if (isSafe == "NO") {
            msg = "웹사이트에 Case 002 악성코드 삽입 감지됨!";
            webhook.send(msg, (err, res) => {
                if (err) {
                    console.log('Error:', err);
                    return;
        ...
        if (isSafe == "NO") {
            msg = "웹사이트에 Case 003 악성코드 삽입 감지됨!"+ msg;
            webhook.send(msg, (err, res) => {
                if (err) {
                    console.log('Error:', err);
                    return;
        ...
        if (isSafe == "NO") {
            msg = "웹사이트에 Case 004 악성코드 삽입 감지됨!" + msg;
            webhook.send(msg, (err, res) => {
                if (err) {
                    console.log('Error:', err);
                    return;
        ...
        console.log(`[${now}] index Is safe? ${isSafe}`);
        await browser.close();
    })();
});